...
public final class XmlGenerator {
	...
	private static void genTemplateBiomeConquestNpc(StringBuilder sb) {
		Collection<LOTRBiomeSpawnList.FactionContainer> conqestContainers = new ArrayList<>();
	...
	private static void genTemplateBiomeMobs(StringBuilder sb) {
		Collection<BiomeGenBase.SpawnListEntry> entries = new ArrayList<>();
	...
	private static void genTemplateBiomeSpawnNpc(StringBuilder sb) {
		Collection<LOTRBiomeSpawnList.FactionContainer> spawnContainers = new ArrayList<>();
	...
	private static void genTemplateFactionConquest(StringBuilder sb) {
		...
		Collection<LOTRBiomeSpawnList.FactionContainer> conquestContainers = new ArrayList<>();
			...
			for (LOTRUnitTradeEntries entries : UNITS) {
				for (LOTRUnitTradeEntry entry : entries.tradeEntries) {
					if (entry.entityClass == entityClass) {
						...
						if (entry.getPledgeType() == LOTRUnitTradeEntry.PledgeType.NONE || entry.alignmentRequired >= 101.0f) {
	...
	private static void genTemplateMobOwner(StringBuilder sb) {
		...
		for (Class<? extends Entity> entityClass : HIREABLE) {
			loop:
			for (Map.Entry<Class<? extends Entity>, Entity> ownerEntry : CLASS_TO_ENTITY_OBJ.entrySet()) {
	...
	private static void genTemplateMobSpawn(StringBuilder sb) {
		...
		Collection<LOTRInvasions.InvasionSpawnEntry> invasionEntries = new ArrayList<>();
